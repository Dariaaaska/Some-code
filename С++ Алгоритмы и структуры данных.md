...
## Обход в глубину - алгоритм BFS

Базовый алгоритм bfs, который далее можно менять в зависимости от конкретной задачи:
...
bool bfs(int v, const std::vector<std::vector<int>>& graph, std::vector<int>& used, std::vector<int>& parent,
